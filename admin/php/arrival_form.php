...
function get_button($link,$id){
    ...
    $sql="SELECT status FROM $GLOBALS[table] where a_id=$id";
    $result = mysqli_query($link, $sql);
    if($result && mysqli_num_rows($result) > 0){ 
        $row = mysqli_fetch_array($result);
        if($row['status']!=0){
            return '<a data-toggle="modal" data-target="#delmodal" class="text-danger" style="cursor:pointer;text-decoration:none;float:right;"><i class="fas fa-fw fa-trash"></i></a>';
        }
    }
    return '<span class="text-warning" onclick="stat('.$id.',-1)" style="cursor:pointer;">Deny</span> | <span class="text-info" onclick="stat('.$id.',1)" style="cursor:pointer;">Confirm</span>
    <a data-toggle="modal" data-target="#delmodal" class="text-danger" style="cursor:pointer;text-decoration:none;float:right;"><i class="fas fa-fw fa-trash"></i></a>';
