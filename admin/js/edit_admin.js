...
const elements = [
    ...
    "contno",
    "dte"];
const emps_str=["emp2","emp3","emp4","emp5","emp8","emp9","emp10"];
const emps = [];
const obj_elements = [];
...
function save(id) {
    if (!einterc && !pinterc) {
        clear_mod();
        if(set_err(["fname","lname","mname","email","contno","dte","addr"],true)){
                return;
        }
    ...
    const columns=["fname","lname","mname","address","email","phone","bday"];
    const types=["string","string","string","string","string","string","date"];
...
function saveinfo(id) {
    ...
    if(passwordValidate("pass","emp11")){
     ...
     if (pass.value != conpass.value) {
        ...
        formdata.append("id", id);
        xhttp.open("POST", "./php/edtacc.php?fun=chngpass&user_type=admin", true);
        xhttp.send(formdata);
...
function delacc(id) {
    ...
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            console.log(this.responseText);
            if (Number(this.responseText) == 1) {
                window.location.href = "acclist.html";
            }
